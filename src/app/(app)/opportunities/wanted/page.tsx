...
export default async function WantedPage({ searchParams }: PageProps<"/opportunities/wanted">) {
  ...
  const leads = await listWatchLeads("wanted", status);
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-slate-500">
        أشخاص يبحثون عن ساعات ثمينة معينة على حراج — فرصة بيع من مخزون المحل. تواصل معهم عبر رابط حراج مباشرة.
      </p>

      <OpportunitiesTabs active="wanted" />
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        {[
          ...
          <a
            key={f.label}
            href={f.key ? `/opportunities/wanted?status=${f.key}` : "/opportunities/wanted"}
      ...
      <div className="space-y-3">
        {leads.map((lead) => (
          <LeadCard key={lead.id} lead={lead} kind="wanted" />
        ))}
        {leads.length === 0 && (
